...
export function DashboardVisualizations({
  ...
  const issued = data?.issuedBooks;
  const overdue = data?.overdueBooks;
  const hasCirculation = issued !== undefined || overdue !== undefined;
  const percentage = seat
    ? (seat.percentage ??
      Math.round((seat.occupied / Math.max(seat.total, 1)) * 100))
    : 0;
  const radius = 42;
  const circumference = 2 * Math.PI * radius;
  const circulationMaximum = Math.max(issued ?? 0, overdue ?? 0, 1);
  return (
    <section className="mt-6 grid gap-6 lg:grid-cols-2">
      <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Current use of available study seats.
        </p>
        {seat ? (
          <div className="mt-5 flex items-center gap-5">
            <svg
              ...
              role="img"
              aria-label={`${percentage}% of seats are in use`}
            >
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {hasCirculation ? (
          <div
            ...
            role="img"
            aria-label={`Issued books: ${issued ?? 0}; overdue books: ${overdue ?? 0}`}
          >
            <CirculationBar
              label="Issued books"
              value={issued ?? 0}
              maximum={circulationMaximum}
              color="bg-indigo-600"
            />
            <CirculationBar
              label="Overdue books"
          ...
          <div className="mt-5 rounded-lg border border-dashed border-slate-300 px-4 py-10 text-center text-sm text-slate-500">
            Circulation data is not available.
          </div>
        )}
      </article>
    </section>
