...
function metrics(data: AdminDashboard): Metric[] {
  ...
  const reservationSummary = data.reservationSummary ?? {
    pending: 0,
    readyForPickup: 0,
  };
  ...
  return [
    ...
    },
    {
      ...
      value: `₹${number(fineSummary.outstandingAmount)}`,
      detail: `${number(fineSummary.pendingPayments)} pending payments`,
    },
    {
      title: "Reservations",
      value: number(reservationSummary.pending),
      detail: `${number(reservationSummary.readyForPickup)} ready for pickup`,
    },
    {
      title: "Seat utilization",
      value: `${seatUtilization.percentage}%`,
      detail: `${number(seatUtilization.occupied)} of ${number(seatUtilization.total)} seats in use`,
    },
  ];
}
function MetricCard({
