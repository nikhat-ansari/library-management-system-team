...
const filterFor = (type: ReportType) =>
  type === "books"
    ? ["category"]
    : type === "members"
      ? ["status", "memberType"]
...
const labelsByField: Record<string, string> = {
  ...
  status_inactive: "Inactive Members",
  total: "Total Records",
...
export function AdminReportsPage() {
  ...
  const report = useQuery({
    queryKey: ["admin", "reports", type, filters, filterRevision],
    queryFn: () => adminReportsService.getReport(type, filters),
    retry: 1,
  });
  const ai = useMutation({
    mutationFn: () => adminReportsService.getAiSummary(type, filters),
    onSuccess: () => setAiUnavailable(""),
    onError: (error) =>
      setAiUnavailable(
        axios.isAxiosError(error) && error.response?.status === 503
          ? "AI summary is currently unavailable. Your official report data is still available."
  ...
  const download = async (format: "csv" | "xlsx" | "pdf") => {
    ...
    setExportError("");
    try {
      const response = await adminReportsService.export(type, format, filters);
      ...
      console.error("Module 6 report export failed", {
        format,
        reportType: type,
        filters,
        error,
      });
      setExportError("Export could not be generated. Please try again.");
    } finally {
  ...
  return (
    <AdminLayout>
      <StaffPageHeader
        title="Reports"
        description="Review official library operations data and download the current filtered report."
      />
      <section className="mt-7 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <label key={field} className="form-label">
              ...
              {field === "status" ? (
                <select
                  value={draft.status ?? ""}
                  onChange={(e) =>
                    setDraft({ ...draft, status: e.target.value || undefined })
                  }
                  className="form-input mt-1"
                >
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDraft({
                      ...draft,
                      memberType: e.target.value || undefined,
                    })
                  }
                  className="form-input mt-1"
                >
                  <option value="">All member types</option>
                  {["STUDENT", "TEACHER", "FACULTY", "EMPLOYEE", "GENERAL"].map(
                    (value) => (
                      <option key={value} value={value}>
                ...
                <input
                  value={draft[field as keyof ReportFilters] ?? ""}
                  onChange={(e) =>
                    setDraft({ ...draft, [field]: e.target.value || undefined })
                  }
                  className="form-input mt-1"
                />
              )}
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <button
            type="button"
            onClick={apply}
            className="min-h-11 rounded-lg bg-indigo-700 px-4 text-sm font-semibold text-white hover:bg-indigo-800"
          >
            Apply filters
          </button>
          <button
          ...
          <button
            ...
            onClick={() => ai.mutate()}
            className="min-h-11 rounded-lg border border-indigo-300 px-4 text-sm font-semibold text-indigo-800"
          >
            {ai.isPending ? "Generating…" : "Generate AI Summary"}
          </button>
        </div>
        {validation && (
        ...
        <>
          ...
          <section className="mt-6 rounded-xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-200 px-5 py-4">
              <h2 className="font-semibold">{labels[type]} report</h2>
              <p className="mt-1 text-sm text-slate-500">
                Generated{" "}
                {report.data?.generatedAt
                  ? new Date(report.data.generatedAt).toLocaleString()
                  : ""}
              </p>
            </div>
            {rows.length === 0 ? (
                      ...
                      >
                        {columns.map((column) => (
                          <td
                            ...
                            className={`max-w-64 px-4 py-3 text-slate-700 ${column === "id" ? "truncate font-mono text-xs" : ""}`}
                          >
                            {column.endsWith("At")
