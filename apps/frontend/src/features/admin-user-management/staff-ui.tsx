...
export function StaffRoleBadge({ role }: { role: StaffRole }) {
  return <span className="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-semibold text-slate-700">{role === 'STAFF' ? 'Librarian / Staff' : role}</span>;
}
