...
export function StaffEditPage() {
  ...
  const query = useQuery({ queryKey: ['admin-users', staffId], queryFn: () => adminUserManagementService.getStaffById(staffId), enabled: Boolean(staffId), retry: 1 });
  const mutation = useMutation({ mutationFn: (values: StaffFormValues) => adminUserManagementService.updateStaff(staffId, values), onSuccess: async (staff) => { await queryClient.invalidateQueries({ queryKey: ['admin-users'] }); await queryClient.invalidateQueries({ queryKey: ['admin-users', staffId] }); navigate(`/admin/users/${staff.id}`, { replace: true, state: { notice: `${staff.name}'s account information was updated.` } }); }, onError: (error) => setServerError(toStaffApiError(error, 'Staff account could not be updated. Please try again.')) });
