...
export class AdminPermissionsController {
  ...
  @Put('users/:id/permissions') @HttpCode(HttpStatus.OK) @ApiOperation({ summary: 'Replace a librarian/staff permission set', description: 'Requires an authenticated ADMIN user. Duplicate or unknown codes are rejected.' }) @ApiParam({ name: 'id' }) @ApiOkResponse() @ApiBadRequestResponse() @ApiNotFoundResponse() @ApiUnauthorizedResponse() @ApiForbiddenResponse() replaceUserPermissions(@Param('id') id: string, @Body() dto: ReplacePermissionsDto, @CurrentUser() user: { userId: string }) { return this.service.replaceUserPermissions(id, dto.permissions, user.userId); }
}
